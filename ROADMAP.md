...
- [ ] Stage 8 — WebSockets
  - [ ] `concepts/10-websockets`: `Hub.Run` (scaffolded, TODO gap open, testable with a fake `*Client` — no real socket needed) + `ReadPump`/`WritePump`/`ServeWS` (reference-only via `gorilla/websocket`, exercised for real in `projects/03-chat-server`)
